...
import { getAllMutasiStockCasting } from "../../../../../application/actions/mutasistockcasting";
...
const maptostate = (state) => {
  if (state.form.FormLaporanMutasiStockCasting?.values !== undefined) {
    return {
      initialValues: {
        date: state.form.FormLaporanMutasiStockCasting.values.date,
        kode_jenis_bahan:
          state.form.FormLaporanMutasiStockCasting.values.kode_jenis_bahan,
        user: state.form.FormLaporanMutasiStockCasting.values.user,
        no_job_order:
          state.form.FormLaporanMutasiStockCasting.values.no_job_order,
        all: state.form.FormLaporanMutasiStockCasting.values.all,
      },
    };
  } else {
    return {
...
let FormLaporanMutasiStockCasting = (prop) => {
      ...
      <Row>
        ...
        <Col offset={1}>
          <Button
            ...
            htmltype="button"
            loading={btnLoading}
            onClick={() => prop.dispatch(getAllMutasiStockCasting)}
            style={{ marginTop: 29 }}
          >
            Lihat Laporan
          </Button>
...
FormLaporanMutasiStockCasting = reduxForm({
  form: "FormLaporanMutasiStockCasting",
  enableReinitialize: true,
})(FormLaporanMutasiStockCasting);
export default connect(maptostate, null)(FormLaporanMutasiStockCasting);
