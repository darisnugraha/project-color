...
import {
  ...
  setDataKartuJobOrderSuccess,
  setDataKartuJobOrderFailed,
...
const getAllDataKartuJobOrder =
  ...
  async (action) => {
    ...
    if (action.type === GET_ALL_KARTU_JOB_ORDER) {
      ...
      dispatch(setDataKartuJobOrderSuccess({ feedback: [] }));
      const data = getState().form.FormLaporanKartuJobOrder.values;
      ...
      if (data.date === null) {
        ...
        writeLocal("kartu_job_order", data);

        const response = await api.KartuJobOrder.getAllKartuJobOrder({
          params: data,
        });
        if (response?.value !== null) {
          ...
          if (response?.value.length === 0) {
            ...
            dispatch(
              setDataKartuJobOrderSuccess({ feedback: response?.value })
            );
          }
        } else {
          dispatch(setLoadingButton(false));
          sweetalert.default.Failed("Terjadi Kesalahan Saat Mengambil Data !");
          dispatch(setDataKartuJobOrderFailed({ error: response.error }));
        }
      }
    }
  };
