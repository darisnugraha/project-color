...
import {
  GET_ALL_KIRIM_CASTING,
  setDataKirimCastingSuccess,
  setDataKirimCastingFailed,
  ...
  async (action) => {
    ...
    if (action.type === GET_ALL_KIRIM_CASTING) {
      ...
      if (data.kode_jenis_bahan === undefined || data.date === null) {
        ...
        writeLocal("kirim_casting_head", data);
        
        const response = await api.KirimCasting.getAllKirimCasting({
          params: data,
        });
        if (response?.value !== null) {
          ...
          if (response?.value.length === 0) {
            ...
            dispatch(setDataKirimCastingSuccess({ feedback: response?.value }));
          }
        } else {
          dispatch(setLoadingButton(false));
          sweetalert.default.Failed("Terjadi Kesalahan Saat Mengambil Data !");
          dispatch(setDataKirimCastingFailed({ error: response.error }));
        }
      }
    }
  };

const data = [kirimcasting];
