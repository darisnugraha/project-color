...
const mutasisaldo =
  ...
  async (action) => {
    next(action);
    if (action.type === GET_ALL_MUTASI_SALDO_BY_DIVISI) {
      ...
      if (data.bahan === undefined) {
        ...
        const response = await api.MutasiSaldo.getAllMutasiSaldo({
          params: data,
        });
        log(response);
        if (response?.value !== null) {
          ...
          if (response?.value.length === 0) {
            ...
            dispatch(
              setDataMutasiSaldoByDivisiSuccess({ feedback: response?.value })
            );
          }
        } else {
          dispatch(setLoadingButton(false));
          sweetalert.default.Failed("Terjadi Kesalahan Saat Mengambil Data !");
          dispatch(setDataMutasiSaldoByDivisiFailed({ error: response.error }));
        }
      }
    }
  };

const data = [mutasisaldo];
