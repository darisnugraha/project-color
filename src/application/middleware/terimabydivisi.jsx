...
import {
  GET_ALL_TERIMA_BY_DIVISI,
  setDataTerimaByDivisiSuccess,
  setDataTerimaByDivisiFailed,
...
const terimabydivisi =
  ...
  async (action) => {
    ...
    if (action.type === GET_ALL_TERIMA_BY_DIVISI) {
      ...
      if (data.divisi === undefined || data.date === null) {
        ...
        writeLocal("terima_by_divisi_head", data);

        const response = await api.TerimaByDivisi.getAllTerimaByDivisi({
          params: data,
        });
        if (response?.value !== null) {
          ...
          if (response?.value.length === 0) {
            ...
            dispatch(
              setDataTerimaByDivisiSuccess({ feedback: response?.value })
            );
          }
        } else {
          dispatch(setLoadingButton(false));
          sweetalert.default.Failed("Terjadi Kesalahan Saat Mengambil Data !");
          dispatch(setDataTerimaByDivisiFailed({ error: response.error }));
        }
      }
    }
  };

const data = [terimabydivisi];
