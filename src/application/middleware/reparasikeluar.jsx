...
import {
  GET_ALL_REPARASI_KELUAR,
  setDataReparasiKeluarSuccess,
  setDataReparasiKeluarFailed,
...
const getDataReparasiMasuk =
  ...
  async (action) => {
    next(action);
    if (action.type === GET_ALL_REPARASI_KELUAR) {
      dispatch(setLoadingButton(true));
      dispatch(setDataReparasiKeluarSuccess([]));
      const data = getState().form.FormGetNoPORK.values;
      const dataPO = getState().form.FormLaporanReparasiKeluar.values;
      if (
        data.date === null ||
        ...
        writeLocal("reparasi_keluar", dataKirim);

        const response = await api.ReparasiKeluar.getAllReparasiKeluar(
          dataKirim
        );
        if (response?.value !== null) {
          ...
          if (response?.value.length === 0) {
            ...
            dispatch(setDataReparasiKeluarSuccess(response?.value));
          }
        } else {
          dispatch(setLoadingButton(false));
          sweetalert.default.Failed("Terjadi Kesalahan Saat Mengambil Data !");
          dispatch(setDataReparasiKeluarFailed(response.error));
        }
      }
    }
  };
