  ...
  async (action) => {
    ...
    if (action.type === GET_ALL_KIRIM_BATU) {
      ...
      if (
        ...
        writeLocal("kirim_batu", data);

        const response = await api.KirimBatu.getAllKirimBatu({
          params: data,
        });
        if (response?.value !== null) {
          ...
          if (response?.value.length === 0) {
            ...
            dispatch(setDataKirimBatuSuccess({ feedback: response?.value }));
          }
        } else {
          dispatch(setLoadingButton(false));
...
const data = [getAllDataKirimBatu];
