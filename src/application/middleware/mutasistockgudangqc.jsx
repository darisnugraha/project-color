...
import {
  GET_ALL_MUTASI_STOCK_GUDANG_QC,
  setDataMutasiStockGudangQCSuccess,
  setDataMutasiStockGudangQCFailed,
} from "../actions/mutasistockgudangqc";
import { setLoadingButton } from "../actions/ui";
import Moment from "moment";
import * as sweetalert from "../../infrastructure/shared/sweetalert";

const getDataMutasiStockGudangQC =
  ({ api, log, writeLocal, getLocal, toast }) =>
  ({ dispatch, getState }) =>
  (next) =>
  async (action) => {
    next(action);
    if (action.type === GET_ALL_MUTASI_STOCK_GUDANG_QC) {
      dispatch(setLoadingButton(true));
      dispatch(setDataMutasiStockGudangQCSuccess({ feedback: [] }));
      const data = getState().form.FormLaporanMutasiStockGudangQC.values;
      if (data.date === null) {
        ...
        const response =
          await api.MutasiStockGudangQC.getAllMutasiStockGudangQC({
            params: data,
          });
        if (response?.value !== null) {
          ...
          dispatch(setDataMutasiStockGudangQCFailed({ error: response.error }));
        }
      }
    }
  };

const data = [getDataMutasiStockGudangQC];
