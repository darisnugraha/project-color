...
import {
  GET_ALL_KIRIM_BARANG_PRODUKSI,
  setDataKirimBarangProduksiSuccess,
  setDataKirimBarangProduksiFailed,
...
const kirimbahanproduksi =
  ...
  async (action) => {
    next(action);
    if (action.type === GET_ALL_KIRIM_BARANG_PRODUKSI) {
      dispatch(setLoadingButton(true));
      dispatch(setDataKirimBarangProduksiSuccess({ feedback: [] }));
      const data = getState().form.FormLaporanKirimBarangProduksi.values;
      ...
      if (data.kode_jenis_bahan === undefined || data.date === null) {
        ...
        if (response?.value !== null) {
          ...
          if (response?.value.length === 0) {
            ...
            dispatch(
              setDataKirimBarangProduksiSuccess({ feedback: response?.value })
            );
          }
        } else {
          dispatch(setLoadingButton(false));
          sweetalert.default.Failed(response.error.data);
          dispatch(setDataKirimBarangProduksiFailed({ error: response.error }));
        }
      }
    }
  };

const data = [kirimbahanproduksi];
