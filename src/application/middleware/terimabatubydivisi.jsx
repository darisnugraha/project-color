...
import {
  GET_ALL_TERIMA_BATU_BY_DIVISI,
  setDataTerimaBatuByDivisiSuccess,
  setDataTerimaBatuByDivisiFailed,
  getDivisi,
...
const getAllTerimaBatuByDivisi =
  ({ api, log, writeLocal, getLocal, toast }) =>
  ({ dispatch, getState }) =>
  (next) =>
  async (action) => {
    next(action);
    if (action.type === GET_ALL_TERIMA_BATU_BY_DIVISI) {
      ...
      if (data.bahan === undefined) {
        ...
        const response = await api.TerimaBatu.getAllTerimaBatu({
          params: data,
        });
        log(response);
        if (response?.value !== null) {
          ...
          if (response?.value.length === 0) {
            ...
            dispatch(
              setDataTerimaBatuByDivisiSuccess({ feedback: response?.value })
            );
          }
        } else {
          dispatch(setLoadingButton(false));
          sweetalert.default.Failed("Terjadi Kesalahan Saat Mengambil Data !");
          dispatch(setDataTerimaBatuByDivisiFailed({ error: response.error }));
        }
      }
    }
  };

const data = [getAllTerimaBatuByDivisi];
