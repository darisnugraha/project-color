...
import {
  GET_ALL_TAMBAH_JOB_ORDER,
  setDataTambahJobOrderSuccess,
  setDataTambahJobOrderFailed,
  ...
  async (action) => {
    ...
    if (action.type === GET_ALL_TAMBAH_JOB_ORDER) {
      ...
      if (data.kode_jenis_bahan === undefined || data.date === null) {
        ...
        const response = await api.TambahJobOrder.getAllTambahJobOrder({
          params: data,
        });
        if (response?.value !== null) {
          ...
          if (response?.value.length === 0) {
            ...
            dispatch(
              setDataTambahJobOrderSuccess({ feedback: response?.value })
            );
          }
        } else {
          dispatch(setLoadingButton(false));
          sweetalert.default.Failed("Terjadi Kesalahan Saat Mengambil Data !");
          dispatch(setDataTambahJobOrderFailed({ error: response.error }));
        }
      }
    }
  };

const data = [tambahjoborder];
